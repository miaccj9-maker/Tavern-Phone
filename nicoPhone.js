...
var CSS = `.Qixian-mu-inp-wrap,.Qixian-mu-ctrl,.Qixian-mu-time-disp,.Qixian-mu-now,.Qixian-mu-stage,.Qixian-mu-invbtn{flex-shrink:0!important;}
...
<div id="Qx-Data-1" style="display:none;">$1</div><div id="Qx-Data-2" style="display:none;">$2</div><div id="Qx-Data-3" style="display:none;">$3</div><div id="Qx-Data-4" style="display:none;">$4</div><div id="Qx-Data-5" style="display:none;">$5</div><div id="Qx-Data-6" style="display:none;">$6</div><div id="Qx-Data-7" style="display:none;">$7</div><div id="Qx-Data-8" style="display:none;">$8</div><div id="Qx-Data-9" style="display:none;">$9</div><div id="Qx-Data-10" style="display:none;">$10</div><div id="Qx-Data-11" style="display:none;">$11</div><div id="Qx-Data-12" style="display:none;">$12</div><div id="Qx-Data-13" style="display:none;">$13</div><div id="Qx-Data-14" style="display:none;">$14</div><div id="Qx-Data-15" style="display:none;">$15</div>`;
function getTavernChar(){
    try{
        var name='', avatar='';
        if(window.character){
            name=window.character.name||'';
            avatar=window.character.avatar||'';
            if(avatar&&!avatar.startsWith('http')){
                try{ avatar=new URL(avatar,window.location.origin).href; }catch(e){}
            }
        }
        var ch=document.querySelector('#avatar,.char-avatar img,.avatar img,img.avatar,.character-avatar img');
        if(ch&&ch.src){ avatar=ch.src; if(!name) name=ch.alt||ch.getAttribute('title')||''; }
        if(!name){
            var nm=document.querySelector('.char-name,#ch_name,.character-name');
            if(nm&&nm.textContent) name=nm.textContent.trim();
        }
        return {name:name||'角色',avatar:avatar||''};
    }catch(e){return {name:'角色',avatar:''};}
}
function getTavernUser(){
    try{
        var name=window.user_name||'我';
        var avatar=window.user_avatar||'';
        if(avatar&&!avatar.startsWith('http')){
            try{ avatar=new URL(avatar,window.location.origin).href; }catch(e){}
        }
        var ua=document.querySelector('#user_avatar,.user-avatar img,.user-icon img,.user-avatar,.user-avatar-icon img');
        if(ua&&ua.src) avatar=ua.src;
        var un=document.querySelector('.user-name,#user_name,[data-testid="user-name"]');
        if(un&&un.textContent) name=un.textContent.trim();
