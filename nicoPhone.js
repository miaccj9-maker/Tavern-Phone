...
.nc-inline-row{display:flex!important;align-items:flex-start;gap:10px;width:100%!important;max-width:100%!important;margin:6px 0;animation:nc-inline-pop .3s ease-out;}
.nc-inline-row.nc-left{justify-content:flex-start;margin-right:auto;}
.nc-inline-row.nc-right{display:flex!important;flex-direction:row-reverse!important;justify-content:flex-end!important;align-items:flex-start!important;width:100%!important;max-width:100%!important;margin:6px 0!important;padding:0!important;float:none!important;clear:both!important;}
...
.nc-inline-bub{padding:9px 14px;border-radius:14px;font-size:14px;line-height:1.55;word-break:break-word;max-width:100%;position:relative;box-shadow:0 1px 2px rgba(0,0,0,.06);}
...
.Nicole-chatlist-item{display:flex;align-items:center;padding:12px 16px;cursor:pointer;border-bottom:1px solid #eee;background:#fff;transition:background .15s;}
...
function initPhone(scope, charInfo, userInfo){
    ...
    Q('.Nicole-jtfact-receive').addEventListener('click',function(){if(window.targetTfNode){window.targetTfNode.classList.add('got');window.targetTfNode.querySelector('.Nicole-tf-t').textContent='已收款';var amt=window.targetTfNode.getAttribute('data-amt')||'0.00';appendCmd(isBlkRight?'$[发送失败]':'$[收款:'+amt+']');playSwoosh();}Q('.Nicole-jtfactmodal').classList.remove('show');});
    Q('.Nicole-jtfact-return').addEventListener('click',function(){if(window.targetTfNode){window.targetTfNode.classList.add('returned');window.targetTfNode.querySelector('.Nicole-tf-t').textContent='已退回';var amt=window.targetTfNode.getAttribute('data-amt')||'0.00';appendCmd(isBlkRight?'$[发送失败]':'$[退回:'+amt+']');playSwoosh();}Q('.Nicole-jtfactmodal').classList.remove('show');});
    ...
    function addCallBubble(dir,text,doType){
        var wrap=document.createElement('div');wrap.className='Nicole-cb-wrap '+dir;var bub=document.createElement('div');bub.className='Nicole-cb';wrap.appendChild(bub);callBubsBox.appendChild(wrap);
    ...
    Q('.Nicole-jbtn-voice').addEventListener('click',function(){openCallUI('voice','out');appendCmd(isBlkRight?'$[呼叫失败，拒收]':'$[呼叫:语音通话]');});
    Q('.Nicole-jbtn-video').addEventListener('click',function(){openCallUI('video','out');appendCmd(isBlkRight?'$[呼叫失败，拒收]':'$[呼叫:视频通话]');});
    ...
    Q('.Nicole-jcall-send').addEventListener('click',function(e){e.stopPropagation();var t=Q('.Nicole-jcall-in').value.trim();if(t){var isVid=Q('.Nicole-jcall').classList.contains('video');addCallBubble('right',t,false);playSwoosh();appendCmd((isVid?'$[视频:':'$[通话:')+t+']');Q('.Nicole-jcall-in').value='';}});
    ...
    function sendText(){var v=textInput.value.trim();if(!v)return;if(pendingReply){renderRight('<div class="Nicole-quote-box">'+pendingReply+'</div>'+v,false,isBlkRight);appendCmd(isBlkRight?'$[拉黑拒收]':(finalRName+' | [引用:'+pendingReply+'] '+v));Q('.Nicole-jrepclose').click();}else{renderRight(v,false,isBlkRight);appendCmd(isBlkRight?'$[消息被拒收]':(finalRName+' | '+v));}textInput.value='';playSwoosh();}
    ...
    function voiceBubble(txt,dur){renderRight('<div class="Nicole-au" data-txt="'+txt+'"><div class="Nicole-au-main"><div class="Nicole-au-play"></div><div class="Nicole-au-bars"><span></span><span></span><span></span><span></span><span></span><span></span><span></span></div><div class="Nicole-au-dur">'+dur+'</div></div><div class="Nicole-au-wrap"><div class="Nicole-au-txt">'+txt+'</div></div></div>',true,isBlkRight);playSwoosh();appendCmd(isBlkRight?'$[发送失败]':'$[语音:'+dur+'|'+txt+']');}
...
function renderChatListScreen(){
    try{
        ...
        list.forEach(function(name){
            ...
            html+='<div class="Nicole-chatlist-item" data-name="'+name+'"><div class="Nicole-chatlist-av"'+avStyle+'>'+(av?'':name.charAt(0))+'</div><div class="Nicole-chatlist-info"><div class="Nicole-chatlist-name">'+name+'</div><div class="Nicole-chatlist-msg">'+lastMsg+'</div></div></div>';
        });
...
function renderPhoneMessage(text,type,isUser){
    try{
        ...
        if(type==='voice'){
            ...
            body='<div class="Nicole-tf"><div class="Nicole-tf-ic">¥</div><div class="Nicole-tf-info"><div class="Nicole-tf-t">'+ttitle+'</div><div class="Nicole-tf-a">¥ '+tamt+'</div><div class="Nicole-tf-f">微信转账</div></div></div>';
        }else{
            body='<div class="Nicole-bub">'+text.replace(/</g,'&lt;').replace(/>/g,'&gt;')+'</div>';
        }
        var row=document.createElement('div');
        row.className='Nicole-row '+dir;
        row.style.opacity='0';
        row.style.animation='Nicole-pop .4s forwards cubic-bezier(.2,.8,.2,1)';
        row.innerHTML='<div class="Nicole-'+(isUser?'rav':'lav')+' '+avClass+'"'+avStyle+'></div><div class="Nicole-ct">'+body+'<div class="Nicole-meta">'+time+(isUser?' <span class="Nicole-tick">✓✓</span>':'')+'</div></div>';
...
function ncInlineBubbleHTML(type,content,isUser){
    ...
    var av='';
    // 优先从手机面板的头像元素读取，确保和手机完全同步
    try{
        var phonePanel=document.getElementById(PANEL_ID);
        if(phonePanel){
            var avEl=isUser?phonePanel.querySelector('.Nicole-bind-rav'):phonePanel.querySelector('.Nicole-bind-lav');
            if(avEl){
                var bgImg=avEl.style.backgroundImage;
                if(!bgImg||bgImg==='none'){bgImg=window.getComputedStyle(avEl).backgroundImage;}
                if(bgImg&&bgImg!=='none'&&bgImg.indexOf('url(')===0){
                    av=bgImg.replace(/^url\(['"]?/,'').replace(/['"]?\)$/,'');
                }
            }
        }
    }catch(e){console.log('[Nicole] 头像读取错误:',e);}
    // 兜底从localStorage读取手动上传的头像
    if(!av){
        try{av=isUser?(localStorage.getItem('Nc-av-right')||''):(localStorage.getItem('Nc-av-left')||'');}catch(e){}
    }
    // 兜底从全局变量
    if(!av){try{av=isUser?safeRAv:safeLAv;}catch(e){}
    }
    try{console.log('[Nicole] 头像 isUser='+isUser+' av='+(av?av.substring(0,50):'空'));}catch(e){}
    // 兜底从全局变量和localStorage读取
    if(!av){try{av=isUser?(safeRAv||localStorage.getItem('Nc-av-right')||''):(safeLAv||localStorage.getItem('Nc-av-left')||'');}catch(e){}}
    if(av==='none') av='';
    // 最后从DOM抓取角色卡头像
    if(!av){
        try{
            if(isUser){
                var uEl=document.querySelector('#user_avatar,.user-avatar img,.user-icon img,.user-avatar,.user-avatar-icon img,[class*="user-avatar"] img');
                if(uEl&&uEl.src&&uEl.src.length>10) av=uEl.src;
            }else{
                var cEl=document.querySelector('#avatar,.char-avatar img,.avatar img,img.avatar,.character-avatar img,[class*="char-avatar"] img');
                if(cEl&&cEl.src&&cEl.src.length>10) av=cEl.src;
            }
        }catch(e){}
    }
...
function ncRenderInlineBubbles(node){
    ...
    html=html.replace(/\[我红包[：:]([^\]]*?)\]/g,function(m,t){return ncInlineBubbleHTML('redpacket',t.trim(),true);});
    html=html.replace(/\[我图片[：:]([^\]]*?)\]/g,function(m,t){return ncInlineBubbleHTML('image',t.trim(),true);});
    html=html.replace(/\[我转账[：:]([^\]]*?)\]/g,function(m,t){return ncInlineBubbleHTML('transfer',t.trim(),true);});
    ...
    if(finalLName&&finalLName!=='角色'&&finalLName!=='Unknown'&&finalLName!=='我'){
        try{
            ...
            html=html.replace(new RegExp('\\['+cnEsc2+'红包[：:]([^\\]]*?)\\]','g'),function(m,t){return ncInlineBubbleHTML('redpacket',t.trim(),false);});
            html=html.replace(new RegExp('\\['+cnEsc2+'图片[：:]([^\\]]*?)\\]','g'),function(m,t){return ncInlineBubbleHTML('image',t.trim(),false);});
            html=html.replace(new RegExp('\\['+cnEsc2+'转账[：:]([^\\]]*?)\\]','g'),function(m,t){return ncInlineBubbleHTML('transfer',t.trim(),false);});
    ...
    html=html.replace(/\[角色红包[：:]([^\]]*?)\]/g,function(m,t){return ncInlineBubbleHTML('redpacket',t.trim(),false);});
    html=html.replace(/\[角色图片[：:]([^\]]*?)\]/g,function(m,t){return ncInlineBubbleHTML('image',t.trim(),false);});
    html=html.replace(/\[角色转账[：:]([^\]]*?)\]/g,function(m,t){return ncInlineBubbleHTML('transfer',t.trim(),false);});
    ...
    html=html.replace(/\[(微信红包|红包)[：:]([^\]]*?)\]/g,function(m,k,t){return ncInlineBubbleHTML('redpacket',t.trim(),isUser);});
    html=html.replace(/\[(微信图片|图片)[：:]([^\]]*?)\]/g,function(m,k,t){return ncInlineBubbleHTML('image',t.trim(),isUser);});
    html=html.replace(/\[(微信转账|转账)[：:]([^\]]*?)\]/g,function(m,k,t){return ncInlineBubbleHTML('transfer',t.trim(),isUser);});
