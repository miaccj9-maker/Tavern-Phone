...
@media (max-width:768px){
    #nicole-float{bottom:calc(16px + env(safe-area-inset-bottom,0px));right:12px;z-index:99999;}
    #nicole-toggle-btn{width:42px;height:42px;}
    ...
    .Nicole-phone{height:100%;max-height:100%;}
}
...
function initPhone(scope, charInfo, userInfo){
    ...
    function addCallBubble(dir,text,doType,callType){
        var box=Q('.Nicole-jcall-bubs');if(!box)return;
...
function scanNodeForPhoneMsg2(node){
    ...
    var hasMarker=/\[(微信|语音|红包|图片|转账|手机|电话|朋友圈|拍一拍|系统|拉黑|加好友|收款|退回|正在输入|角色|我|角色语音|角色红包|角色图片|角色转账|角色表情|我语音|我红包|我图片|我转账|我表情|我朋友圈|我拍一拍|我拉黑|我取消拉黑|我加好友|我电话|我通话|我视频|角色通话|角色视频|通话|视频)[：:]|\[(我拉黑|我取消拉黑)\]|!\[[^\]]*\]\(/.test(txt)||/<msg>/.test(txt)||(finalLName&&finalLName!=='角色'&&finalLName!=='Unknown'&&new RegExp('\\['+finalLName.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')+'(朋友圈|拍一拍|拉黑|取消拉黑|加好友)?[：:]?').test(txt));
    ...
    if((fm=txt.match(/\[我通话[：:]([^\]]*?)\]/))&&fm[1].trim()){try{if(window.NcAddCallBubble)window.NcAddCallBubble('right',fm[1].trim(),false,'voice');}catch(e){}}
    if((fm=txt.match(/\[我视频[：:]([^\]]*?)\]/))&&fm[1].trim()){try{if(window.NcAddCallBubble)window.NcAddCallBubble('right',fm[1].trim(),false,'voice');}catch(e){}}
    ...
    for(var cni=0;cni<charNames.length;cni++){
        try{
            ...
            while((dm=dmCallRe.exec(txt))!==null){if(dm[1].trim()){try{if(window.NcAddCallBubble)window.NcAddCallBubble('left',dm[1].trim(),false,'voice');}catch(e){}}}
            var dmVidRe=new RegExp('\\['+cnEsc+'视频[：:]([^\\]]*?)\\]','g');
            while((dm=dmVidRe.exec(txt))!==null){if(dm[1].trim()){try{if(window.NcAddCallBubble)window.NcAddCallBubble('left',dm[1].trim(),false,'voice');}catch(e){}}}
    ...
    try{
        ...
        while((_gcm=_genCallRe.exec(txt))!==null){
            ...
            if(_gcc){
                var _isMe=(_gcn==='我');
                try{if(window.NcAddCallBubble)window.NcAddCallBubble(_isMe?'right':'left',_gcc,false,'voice');}catch(e){}
            }
        }
    }catch(e){}
    // 通用视频通话中消息：[任意名字视频:内容]
    try{
        ...
        while((_gvm=_genVidRe.exec(txt))!==null){
            ...
            if(_gvc){
                var _isMe2=(_gvn==='我');
                try{if(window.NcAddCallBubble)window.NcAddCallBubble(_isMe2?'right':'left',_gvc,false,'video');}catch(e){}
            }
        }
    }catch(e){}
