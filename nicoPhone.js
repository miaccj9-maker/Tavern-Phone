/* Qixian Phone - SillyTavern Extension v3.0 (基于原始代码完整移植) */
(function(){
'use strict';

var PANEL_ID = 'qixian-float-panel';
var TOGGLE_ID = 'qixian-float-toggle';
var STYLE_ID = 'qixian-phone-styles-v3';

/* ============ CSS (完整保留原版，仅去除全局污染) ============ */
var CSS = `
#qixian-float-panel *{box-sizing:border-box;margin:0;padding:0;}
...
.Qixian-stage ::-webkit-scrollbar{display:none;width:0;height:0;}
...
#qixian-float-toggle{position:fixed;bottom:24px;right:24px;width:52px;height:52px;border-radius:50%;background:rgba(30,30,30,.9);backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px);display:flex;justify-content:center;align-items:center;cursor:pointer;z-index:2147483646;box-shadow:0 4px 20px rgba(0,0,0,.3);transition:transform .2s,opacity .2s;border:1px solid rgba(255,255,255,.1);}
#qixian-float-toggle:active{transform:scale(.9);}
#qixian-float-toggle svg{width:26px;height:26px;stroke:#fff;stroke-width:1.8;fill:none;}
#qixian-float-panel{position:fixed;bottom:88px;right:24px;z-index:2147483645;display:none;transform-origin:bottom right;animation:qixian-float-in .25s cubic-bezier(.2,.8,.2,1);}
#qixian-float-panel.show{display:block;}
@keyframes qixian-float-in{from{opacity:0;transform:scale(.9) translateY(10px);}to{opacity:1;transform:scale(1) translateY(0);}}
`;
...
function getTavernChar(){
    try{
        if(window.character && window.character.name){
            return {name:window.character.name, avatar:window.character.avatar || ''};
        }
    }catch(e){}
    try{
        var av = document.querySelector('#avatar, .char-avatar img, .avatar img, [data-testid="char-avatar"]');
        var nm = document.querySelector('.char-avatar .name, #char_name, [data-testid="char-name"]');
        return {name: nm ? nm.textContent.trim() : 'Character', avatar: av ? av.src : ''};
    }catch(e){ return {name:'Character', avatar:''}; }
}
function getTavernUser(){
    try{
        if(window.user_name){
            return {name:window.user_name, avatar:window.user_avatar || ''};
        }
    }catch(e){}
    try{
        var av = document.querySelector('#user_avatar, .user-avatar img, [data-testid="user-avatar"], .user-icon img');
        return {name:'You', avatar: av ? av.src : ''};
    }catch(e){ return {name:'You', avatar:''}; }
}
...
try {
    ...
    function getVal(id) {
        var el = scope.querySelector('[id="' + id + '"]') || document.getElementById(id);
        return el ? (el.textContent || el.innerText || "").trim() : "";
    }

    // 填充 Qx-Data 占位符 (从酒馆获取)
    var dataMap = {
        'Qx-Data-1': charInfo.avatar || '',
        'Qx-Data-2': charInfo.name || 'Character',
        'Qx-Data-3': userInfo.avatar || '',
        'Qx-Data-4': userInfo.name || 'You',
        'Qx-Data-5': '相恋中',
        'Qx-Data-6': '', 'Qx-Data-7': '', 'Qx-Data-8': '未知',
        'Qx-Data-9': '', 'Qx-Data-10': '', 'Qx-Data-11': '的肩膀',
        'Qx-Data-12': '的脑袋', 'Qx-Data-13': '', 'Qx-Data-14': '',
        'Qx-Data-15': '', 'Qx-Data-16': ''
    };
    Object.keys(dataMap).forEach(function(id){
        var el = scope.querySelector('#'+id);
        if(el) el.textContent = dataMap[id];
    });

    var rawLAv = getVal("Qx-Data-1"), rawLName = getVal("Qx-Data-2"),
        rawRAv = getVal("Qx-Data-3"), rawRName = getVal("Qx-Data-4"),
        rawRel = getVal("Qx-Data-5"), rawCLoc = getVal("Qx-Data-6"),
        rawULoc = getVal("Qx-Data-7"), rawDist = getVal("Qx-Data-8"),
        rawCSign = getVal("Qx-Data-9"), rawUSign = getVal("Qx-Data-10"),
        rawCPat = getVal("Qx-Data-11"), rawUPat = getVal("Qx-Data-12"),
        rawSys = getVal("Qx-Data-13"), rawMsg = getVal("Qx-Data-14"),
        rawPyq = getVal("Qx-Data-15"), rawCpAlbum = getVal("Qx-Data-16");

    var QxStore = {
        get: function(k) { try { return localStorage.getItem(k); } catch(e) { return null; } },
        set: function(k, v) { try{ localStorage.setItem(k, v); } catch(e) {} }
    };
    function resolveSillyTavernAvatar(rawUrl, isChar) {
        if (rawUrl && rawUrl.startsWith('http')) return rawUrl;
        try {
          var doc = document;
          if (isChar) {
          var cImg = doc.querySelector('#avatar, .char-avatar img, .avatar img, [data-testid="char-avatar"], .character-avatar');
          if (cImg && cImg.src) return cImg.src;
          var cBg = doc.querySelector('.char-avatar, .character-avatar');
          if (cBg) {
          var bg = window.getComputedStyle(cBg).backgroundImage;
          var m = bg.match(/url\(["']?([^"']+)["']?\)/);
          if (m && m[1]) return m[1];
          }
          } else {
          var uImg = doc.querySelector('#user_avatar, .user-avatar img, [data-testid="user-avatar"], .user-icon img');
          if (uImg && uImg.src) return uImg.src;
          var uBg = doc.querySelector('.user-avatar, .user-icon');
          if (uBg) {
          var ubg = window.getComputedStyle(uBg).backgroundImage;
          var um = ubg.match(/url\(["']?([^"']+)["']?\)/);
    ...
    var finalLAv = resolveSillyTavernAvatar(rawLAv, true) || '';
    var finalRAv = resolveSillyTavernAvatar(rawRAv, false) || '';
    var safeLAv = finalLAv ? finalLAv.replace(/"/g, '&quot;').replace(/'/g, '%27') : '';
    var safeRAv = finalRAv ? finalRAv.replace(/"/g, '&quot;').replace(/'/g, '%27') : '';
    QA('.Qixian-bind-lav').forEach(function(el){
        if(safeLAv) el.style.backgroundImage = "url('" + safeLAv + "')";
    });
    QA('.Qixian-bind-rav').forEach(function(el){
        if(safeRAv) el.style.backgroundImage = "url('" + safeRAv + "')";
    });
    QA('.Qixian-bind-lav-bg').forEach(function(el){
        if(safeLAv) el.style.backgroundImage = "url('" + safeLAv + "')";
    });
    QA('.Qixian-bind-rav-bg').forEach(function(el){
        if(safeRAv) el.style.backgroundImage = "url('" + safeRAv + "')";
    });
    ...
    var finalCPat = rawCPat || '的肩膀';
    var finalUPat = rawUPat || '的脑袋';
    var pendingReply = '';
    var actx = null;
    ...
    var phoneContacts = window.QxContactsDB || [{ id: '1', name: finalLName, phone: '13800138000' }, { id: '2', name: finalRName + ' (我)', phone: '13900139000' }];
    window.QxContactsDB = phoneContacts;
    var phoneHistory = window.QxHistoryDB || [];
    window.QxHistoryDB = phoneHistory;
    setInterval(function(){
        var d = new Date();
        var t = Q('.Qixian-jhome-time');
        if(t) t.textContent = String(d.getHours()).padStart(2,'0')+':'+String(d.getMinutes()).padStart(2,'0');
    }, 1000);
    var stickyTxt = Q('.Qixian-jsticky-txt');
    var stickyBtn = Q('.Qixian-jsticky-save');
    if (stickyTxt && stickyBtn) {
        var savedSticky = QxStore.get('Qx-sticky-note');
        if(savedSticky) stickyTxt.value = savedSticky;
        stickyBtn.addEventListener('click', function(){
          var val = stickyTxt.value; QxStore.set('Qx-sticky-note', val); playSwoosh();
          appendCmd('$[更新便签:' + val + ']');
    ...
    QA('.Qixian-ptab').forEach(function(tab){
        tab.addEventListener('click', function(){ QA('.Qixian-ptab').forEach(function(t){ t.classList.remove('active'); }); this.classList.add('active'); renderPhoneTab(this.getAttribute('data-target')); });
    });
    function renderPhoneTab(tabType) {
        ...
        if(tabType === 'recents') {
          ...
          var html = '';
          phoneHistory.forEach(function(h){ html += '<div class="Qixian-list-item"><div><div class="Qixian-item-title" style="color:'+(h.type==='out'?'#222':'#555')+';">'+h.name+'</div><div class="Qixian-item-sub">手机 - '+h.time+'</div></div><div class="Qixian-item-arrow"><svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"></circle><line x1="12" y1="16" x2="12" y2="12"></line><line x1="12" y1="8" x2="12.01" y2="8"></line></svg></div></div>'; });
          phoneContent.innerHTML = html;
          }
        } else if(tabType === 'contacts') {
          var cHtml = '';
          phoneContacts.forEach(function(c, i){ cHtml += '<div class="Qixian-list-item j-contact" data-idx="'+i+'"><div class="Qixian-item-title">'+c.name+'</div></div>'; });
          phoneContent.innerHTML = cHtml;
          QA('.j-contact').forEach(function(el){
          el.addEventListener('click', function(){ var idx = parseInt(this.getAttribute('data-idx')); renderContactDetail(phoneContacts[idx], idx); });
          });
          ...
          keys.forEach(function(k){ dHtml += '<div class="Qixian-dial-key">'+k+'</div>'; });
          ...
          phoneContent.innerHTML = dHtml;
          var ddisp = Q('#dial-disp');
          QA('.Qixian-dial-key').forEach(function(el){
          el.addEventListener('click', function(){ ddisp.textContent += this.textContent; playSwoosh(); });
          });
          Q('.Qixian-dial-callbtn').addEventListener('click', function(){
          var num = ddisp.textContent; if(!num) return;
          var matchContact = phoneContacts.find(function(c) { return c.phone === num; });
          triggerCall(matchContact ? matchContact.name : num, num);
          });
        }
    }
    function renderContactDetail(contact, idx) {
        ...
        phoneContent.innerHTML = html;
        Q('.j-c-save').addEventListener('click', function(){
          var nn = Q('#c-edit-name').value.trim(); var np = Q('#c-edit-phone').value.trim();
          if(nn && np) { phoneContacts[idx].name = nn; phoneContacts[idx].phone = np; playSwoosh(); renderPhoneTab('contacts'); }
        });
    ...
    colorMap.forEach(function(c){
      try {
        ...
        if(picker && txtInp) {
          ...
          if(savedColor) {
          mainStage.style.setProperty(c.v, savedColor, 'important');
          picker.value = (savedColor.length === 7) ? savedColor : '#ffffff';
          txtInp.value = savedColor;
          }
    ...
    if(btnRnd && btnSq) {
      btnRnd.addEventListener('click', function(){ root.classList.remove('av-sq'); btnRnd.classList.add('active'); btnSq.classList.remove('active'); QxStore.set('Qx-av-shape', 'rnd'); });
      btnSq.addEventListener('click', function(){ root.classList.add('av-sq'); btnSq.classList.add('active'); btnRnd.classList.remove('active'); QxStore.set('Qx-av-shape', 'sq'); });
    }
    ...
    if(btnGlass && btnSolid) {
      btnGlass.addEventListener('click', function(){ root.classList.remove('solid-mode'); btnGlass.classList.add('active'); btnSolid.classList.remove('active'); QxStore.set('Qx-glass-mode', 'glass'); });
      btnSolid.addEventListener('click', function(){ root.classList.add('solid-mode'); btnSolid.classList.add('active'); btnGlass.classList.remove('active'); QxStore.set('Qx-glass-mode', 'solid'); });
    }
    ...
    if(bgUpload) {
      bgUpload.addEventListener('click', function(){
          ...
          var sfB64 = b64.replace(/"/g, '&quot;').replace(/'/g, '%27');
          QA('.Qixian-jbg, .Qixian-jhome').forEach(function(el){ el.style.backgroundImage = 'url(\'' + sfB64 + '\')'; });
          if(!Q('.Qixian-jcall').classList.contains('video')) { QA('.Qixian-call-vbg').forEach(function(el){ el.style.backgroundImage = 'url(\'' + sfB64 + '\')'; }); }
          };
    ...
    if(bgClear) {
      bgClear.addEventListener('click', function(){
          ...
          QA('.Qixian-jbg, .Qixian-jhome').forEach(function(el){ el.style.backgroundImage = 'none'; });
          if(!Q('.Qixian-jcall').classList.contains('video')) { QA('.Qixian-call-vbg').forEach(function(el){ el.style.backgroundImage = 'none'; }); }
      });
    }
    ...
    QA('.Qixian-uname').forEach(function(el){
      ...
      var isL = this.closest('.Qixian-jpat-l'); var nV = this.textContent.trim();
      if(isL && nV && nV !== finalLName) { finalLName = nV; appendCmd('$['+finalRName+' 修改了 '+finalLName+' 的备注:'+nV+']'); renderSysMsg(finalRName + ' 将 ' + finalLName + ' 备注修改为 "'+nV+'"'); }
      else if(!isL && nV && nV !== finalRName) { finalRName = nV; appendCmd('$['+finalRName+' 修改了自己的名字:'+nV+']'); renderSysMsg(finalRName + ' 将自己的名字修改为 "'+nV+'"'); }
      });
    });
    ...
    var zoom = Q('.Qixian-jtxtzoom'), zoomIn = Q('.Qixian-jtxtzoomin'); var lastClickTime = 0, lastClickTarget = null, targetRevokeRow = null, targetRevokeText = '', targetBubNode = null; window.targetTfNode = null;
    chatBox.addEventListener('click', function(e){
        ...
        if (isViewRev) {
          e.stopPropagation();
          Q('.Qixian-jviewtxt').value = isViewRev.getAttribute('data-txt') || '';
          Q('.Qixian-jviewmodal').classList.add('show');
          return;
        }
    ...
    Q('.Qixian-jact-cancel').addEventListener('click', function(){ Q('.Qixian-jmsgact').classList.remove('show'); }); Q('.Qixian-jact-reply').addEventListener('click', function(){ showReplyBar(targetRevokeText.substring(0,30)); Q('.Qixian-jmsgact').classList.remove('show'); });
    ...
    Q('.Qixian-jtxtzoom').addEventListener('click', function(e){ if(e.target === this) this.classList.remove('show'); }); Q('.Qixian-jviewclose').addEventListener('click', function(){ Q('.Qixian-jviewmodal').classList.remove('show'); });
    ...
    function saveDrawState() { drawHistory.push(drawCanvas.toDataURL()); if(drawHistory.length > 20) drawHistory.shift(); } function resetDrawBoard(){ drawCtx.globalCompositeOperation = 'source-over'; drawCtx.fillStyle = '#ffffff'; drawCtx.fillRect(0, 0, drawCanvas.width, drawCanvas.height); drawHistory = []; }
    ...
    Q('.Qixian-jdrawclear').addEventListener('click', resetDrawBoard); Q('.Qixian-jdrawundo').addEventListener('click', function(){ if(drawHistory.length > 0) { var img = new Image(); img.src = drawHistory.pop(); img.onload = function() { drawCtx.globalCompositeOperation = 'source-over'; drawCtx.clearRect(0,0,drawCanvas.width,drawCanvas.height); drawCtx.drawImage(img,0,0); }; } else { resetDrawBoard(); } }); Q('.Qixian-jdraweraser').addEventListener('click', function(){ isEraser = !isEraser; this.style.background = isEraser ? '#eeeeee' : ''; this.style.color = isEraser ? '#333' : ''; });
    Q('.Qixian-jdrawcancel').addEventListener('click', function(){ Q('.Qixian-jdrawmodal').classList.remove('show'); }); Q('.Qixian-jdrawok').addEventListener('click', function(){ drawCtx.globalCompositeOperation = 'source-over'; var b64 = drawCanvas.toDataURL('image/jpeg', 0.6); var shortId = 'local-draw-' + Date.now(); try { QxStore.set('Qx-' + shortId, b64); } catch(e){} renderRight('<img src="'+b64+'" class="Qixian-img" alt="手绘涂鸦">', true, isBlkRight); playSwoosh(); appendCmd(isBlkRight ? '$[发送失败]' : '$[图:'+shortId+'|手绘涂鸦]'); Q('.Qixian-jdrawmodal').classList.remove('show'); });

    var callIntv = null, callSec = 0, callState = 'none'; function formatTime(sec){ var m = String(Math.floor(sec/60)).padStart(2,'0'); var s = String(sec%60).padStart(2,'0'); return m+':'+s; }
    function openCallUI(type, state) {
        ...
        if (type === 'video') {
          ...
          if (savedBg && savedBg !== 'none') { Q('.Qixian-call-vbg').style.backgroundImage = 'url(\''+savedBg.replace(/'/g,'%27')+'\')'; }
          else if (defaultBg && defaultBg !== 'none') { Q('.Qixian-call-vbg').style.backgroundImage = defaultBg; }
          else { Q('.Qixian-call-vbg').style.backgroundImage = ''; }
        }
        callSec = 0; callState = state; clearInterval(callIntv);
        var panel = Q('.Qixian-jpanel'); var plusBtn = Q('.Qixian-jplus');
        if(panel) panel.classList.remove('show'); if(plusBtn) plusBtn.classList.remove('on');
    }
    function setActiveCall(forceType) {
        ...
        callIntv = setInterval(function(){ callSec++; Q('.Qixian-jcall-timer').textContent = formatTime(callSec); }, 1000);
    }
    function closeCall() {
    ...
    function addCallBubble(dir, text, doType) {
        var wrap = document.createElement('div'); wrap.className = 'Qixian-cb-wrap ' + dir;
        var bub = document.createElement('div'); bub.className = 'Qixian-cb';
        wrap.appendChild(bub); callBubsBox.appendChild(wrap);
        if(doType) {
          var i = 0; var timer = setInterval(function() {
          if(i < text.length) { bub.innerHTML += text.charAt(i); i++; callBubsBox.scrollTop = callBubsBox.scrollHeight; }
          else { clearInterval(timer); }
          }, 60);
        } else { bub.innerHTML = text; setTimeout(function(){ callBubsBox.scrollTop = callBubsBox.scrollHeight; }, 50); }
    }
    ...
    Q('.Qixian-jcall-send').addEventListener('click', function(e){
        e.stopPropagation(); var t = Q('.Qixian-jcall-in').value.trim();
        if(t) {
          var isVid = Q('.Qixian-jcall').classList.contains('video');
          addCallBubble('right', t, false); playSwoosh();
    ...
    function cDragEnd(e) { cInitX = cCurrX; cInitY = cCurrY; cDragging = false; }
    ...
    var baseEmojiArr = [  { i: 'https://tuchuang.org.cn/imgs/2026/03/26/8abc1e15982dce90.png', t: '宝宝我惹你了吗？' },
          ...
          { i: 'https://tuchuang.org.cn/imgs/2026/03/27/b68c231476fd9735.png', t: '一张古早叼花漫画图' },
          { i: 'https://tuchuang.org.cn/imgs/2026/03/27/29907ae552edde90.png', t: '一张抽象的人物图' },
          { i: 'https://tuchuang.org.cn/imgs/2026/03/27/a082ed0adae88380.png', t: '一张抽象龙图拿着杯子' },
          { i: 'https://tuchuang.org.cn/imgs/2026/03/27/ca976c11387dc5e8.png', t: '一张龙图鄙夷的表情' },
          { i: 'https://tuchuang.org.cn/imgs/2026/03/27/422696afc246a494.png', t: '一只猴子苍蝇搓手' },
          { i: 'https://tuchuang.org.cn/imgs/2026/03/27/c21b68dbdf340f36.png', t: '一张搞怪龙图' },
          { i: 'https://tuchuang.org.cn/imgs/2026/03/28/f369ba2676f4283c.png', t: '沸羊羊耍帅' },
          { i: 'https://tuchuang.org.cn/imgs/2026/03/28/be2862ca6b4b0c3a.png', t: '一个抽象的简笔画' },
    ...
    Q('.Qixian-jaddemobtn').addEventListener('click', function(){ Q('.Qixian-jaddemomodal').classList.add('show'); Q('.Qixian-jemomodal').classList.remove('show'); }); Q('.Qixian-jaddemocancel').addEventListener('click', function(){ Q('.Qixian-jaddemomodal').classList.remove('show'); Q('.Qixian-jemomodal').classList.add('show'); }); Q('.Qixian-jaddemook').addEventListener('click', function(){ var u = Q('.Qixian-jaddemourl').value.trim(), t = Q('.Qixian-jaddemotxt').value.trim() || '自定义表情'; if(u) { customEmoArr.unshift({i:u, t:t}); QxStore.set('Qx-custom-emos', JSON.stringify(customEmoArr)); renderEmoList(); Q('.Qixian-jaddemourl').value=''; Q('.Qixian-jaddemotxt').value=''; } Q('.Qixian-jaddemomodal').classList.remove('show'); Q('.Qixian-jemomodal').classList.add('show'); });
    ...
    function sendText(){ var v = textInput.value.trim(); if(!v) return; if(pendingReply) { renderRight('<div class="Qixian-quote-box">' + pendingReply + '</div>' + v, false, isBlkRight); appendCmd(isBlkRight ? '$[拉黑拒收]' : (finalRName + ' | [引用:' + pendingReply + '] ' + v)); Q('.Qixian-jrepclose').click(); } else { renderRight(v, false, isBlkRight); appendCmd(isBlkRight ? '$[消息被拒收]' : (finalRName + ' | '+v)); } textInput.value=''; playSwoosh(); } sendBtn.addEventListener('click', sendText);
    textInput.addEventListener('keydown', function(e){ if(e.key==='Enter'){ e.preventDefault(); sendText(); } });
    var plusBtn = Q('.Qixian-jplus'), panel = Q('.Qixian-jpanel'); plusBtn.addEventListener('click', function(){ plusBtn.classList.toggle('on'); panel.classList.toggle('show'); });
    function openModal(sel){ Q(sel).classList.add('show'); panel.classList.remove('show'); plusBtn.classList.remove('on'); }
    Q('.Qixian-jgiftbtn').addEventListener('click', function(){ openModal('.Qixian-jgiftmodal'); }); Q('.Qixian-jgiftcancel').addEventListener('click', function(){ Q('.Qixian-jgiftmodal').classList.remove('show'); }); Q('.Qixian-jlinkbtn').addEventListener('click', function(){ openModal('.Qixian-jlinkmodal'); }); Q('.Qixian-jlinkcancel').addEventListener('click', function(){ Q('.Qixian-jlinkmodal').classList.remove('show'); }); Q('.Qixian-jtf').addEventListener('click', function(){ openModal('.Qixian-jtfmodal'); }); Q('.Qixian-jtfcancel').addEventListener('click', function(){ Q('.Qixian-jtfmodal').classList.remove('show'); }); Q('.Qixian-jemo').addEventListener('click', function(){ openModal('.Qixian-jemomodal'); }); Q('.Qixian-jemoclose').addEventListener('click', function(){ Q('.Qixian-jemomodal').classList.remove('show'); }); Q('.Qixian-jmusic').addEventListener('click', function(){ openModal('.Qixian-jmumodal'); }); Q('.Qixian-jmuclose').addEventListener('click', function(){ Q('.Qixian-jmumodal').classList.remove('show'); }); Q('.Qixian-jcp').addEventListener('click', function(){ openModal('.Qixian-jcpmodal'); }); Q('.Qixian-jcpclose').addEventListener('click', function(){ Q('.Qixian-jcpmodal').classList.remove('show'); }); Q('.Qixian-jtxtimg').addEventListener('click', function(){ openModal('.Qixian-jtxtimgmodal'); }); Q('.Qixian-jtxtimgcancel').addEventListener('click', function(){ Q('.Qixian-jtxtimgmodal').classList.remove('show'); Q('.Qixian-jtxtimgin').value=''; }); Q('.Qixian-jimgbtn').addEventListener('click', function(){ openModal('.Qixian-jimgmodal'); }); Q('.Qixian-jimgcancel').addEventListener('click', function(){ Q('.Qixian-jimgmodal').classList.remove('show'); Q('.Qixian-jimgurl').value=''; Q('.Qixian-jimgdesc').value=''; }); Q('.Qixian-jbtn-loc').addEventListener('click', function(){ openModal('.Qixian-jlocmodal'); }); Q('.Qixian-jlocclose').addEventListener('click', function(){ Q('.Qixian-jlocmodal').classList.remove('show'); }); Q('.Qixian-jbtn-food').addEventListener('click', function(){ openModal('.Qixian-jfoodmodal'); }); Q('.Qixian-jfoodcancel').addEventListener('click', function(){ Q('.Qixian-jfoodmodal').classList.remove('show'); });
    ...
    Q('.Qixian-jlocsend').addEventListener('click', function(){ Q('.Qixian-jlocinputmodal').classList.add('show'); }); Q('.Qixian-jlocincancel').addEventListener('click', function(){ Q('.Qixian-jlocinputmodal').classList.remove('show'); });
    ...
    function setupSR() {
        ...
        try {
          ...
          window.voiceObj.onresult = function(ev){ for(var i=ev.resultIndex; i<ev.results.length; i++) { if(ev.results[i].isFinal) finalTxt += ev.results[i][0].transcript; } };
          window.voiceObj.onerror = function(ev){ if(ev.error !== 'no-speech') { isRec=false; micBtn.classList.remove('rec'); fallback(ev.error==='not-allowed'?'权限被拒':'识别被中断'); } };
          window.voiceObj.onend = function(){ if(isRec) { try{ window.voiceObj.start(); }catch(e){} } else { micBtn.classList.remove('rec'); if(finalTxt.trim()) voiceBubble(finalTxt.trim(), Math.max(1, Math.round(finalTxt.length/4))+'"'); } };
          return true;
    ...
    function startMediaRecord() {
        if(pNav.mediaDevices && pNav.mediaDevices.getUserMedia) {
          ...
          mediaRec.onstop = function(){ var dur = Math.max(1, Math.round((Date.now() - recStartTime)/1000)); stream.getTracks().forEach(function(t){ t.stop(); }); tempVoiceDur = dur + '"'; fallback('语音录制完毕 (真实时长: ' + dur + '秒)'); };
          recStartTime = Date.now(); mediaRec.start(); isRec = true; micBtn.classList.add('rec');
          }).catch(function(){ fallback('麦克风被占用或拒绝'); });
        } else { fallback('浏览器环境不支持录音'); }
    }
    ...
    Q('.Qixian-jpyqsendok').addEventListener('click',function(){
        ...
        if(t||i||ti){
          playSwoosh();var payloadI=i?i:(ti?'text:'+ti:'');appendCmd('$[发布朋友圈:'+t+'|'+payloadI+']');
          var newId='new_'+Date.now();var pyqUavImg=QxStore.get('Qx-pyq-uav')||safeRAv;
          var pyqStyle=pyqUavImg?' style="background-image:url(\''+pyqUavImg.replace(/'/g,'%27')+'\')"':'';
          var h='<div class="Qixian-pyq-item" data-txt="'+(t||(ti?'[文字图]':'[图片动态]'))+'"><div class="Qixian-pyq-delbtn jpyqdel" data-id="'+newId+'" title="删除"><svg viewBox="0 0 24 24"><path d="M3 6h18M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg></div><div class="Qixian-pyq-iav"'+pyqStyle+'></div><div class="Qixian-pyq-ict"><div class="Qixian-pyq-inm">'+finalRName+'</div>';
         if(t) h += '<div class="Qixian-pyq-itxt">'+t+'</div>';
          if(ti) { h += '<div class="Qixian-pyq-txtimg Qixian-txt-img">'+ti+'</div>'; } else if(i) { h += '<img src="'+getRealImgUrl(i)+'" class="Qixian-pyq-iimg">'; }
          h += '<div class="Qixian-pyq-ibot"><span>刚刚</span><div class="Qixian-pyq-iacts"><div class="Qixian-pyq-btn jpyqlike" data-id="'+newId+'"><svg viewBox="0 0 24 24"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg></div><div class="Qixian-pyq-btn jpyqcom" data-id="'+newId+'"><svg viewBox="0 0 24 24"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg></div></div></div></div></div>';
          Q('.Qixian-jpyqlist').insertAdjacentHTML('afterbegin', h);
          var newLike = Q('.Qixian-jpyqlist .jpyqlike[data-id="'+newId+'"]'); var newComBtn = Q('.Qixian-jpyqlist .jpyqcom[data-id="'+newId+'"]'); var newDelBtn = Q('.Qixian-jpyqlist .jpyqdel[data-id="'+newId+'"]');
          newLike.addEventListener('click', function(){ playSwoosh(); appendCmd('$[点赞朋友圈:刚刚]'); this.style.color='#222'; this.querySelector('svg').style.fill='#222'; var item = this.closest('.Qixian-pyq-item'); var ints = item.querySelector('.Qixian-pyq-ints'); if(!ints) { ints = document.createElement('div'); ints.className='Qixian-pyq-ints'; item.querySelector('.Qixian-pyq-ict').appendChild(ints); } var likes = ints.querySelector('.Qixian-pyq-likes'); if(!likes) { likes = document.createElement('div'); likes.className='Qixian-pyq-likes'; likes.innerHTML='<svg viewBox="0 0 24 24"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg></div>'; ints.insertBefore(likes, ints.firstChild); } });
          newComBtn.addEventListener('click', function(){ curPyqIdx = newId; Q('.Qixian-jpyqcomtxt').placeholder = '说点什么...'; Q('.Qixian-jpyqcommodal').classList.add('show'); });
          newDelBtn.addEventListener('click', function(){ var item = this.closest('.Qixian-pyq-item'); item.parentNode.removeChild(item); playSwoosh(); appendCmd('$[删除朋友圈:刚刚]'); });
          Q('.Qixian-jpyqsendmodal').classList.remove('show'); Q('.Qixian-jpyqsendtxt').value=''; Q('.Qixian-jpyqsendimg').value=''; Q('.Qixian-jpyqsendtxtimg').value='';
        }
    });
    Q('.Qixian-jpyqcomcancel').addEventListener('click',function(){Q('.Qixian-jpyqcommodal').classList.remove('show');});
    Q('.Qixian-jpyqcomok').addEventListener('click',function(){var t=Q('.Qixian-jpyqcomtxt').value.trim();if(t){playSwoosh();appendCmd('$[评论朋友圈:'+t+']');var btn=Q('.Qixian-jpyqlist .jpyqcom[data-id="'+curPyqIdx+'"]');if(btn){var item=btn.closest('.Qixian-pyq-item');var ints=item.querySelector('.Qixian-pyq-ints');if(!ints){ints=document.createElement('div');ints.className='Qixian-pyq-ints';item.querySelector('.Qixian-pyq-ict').appendChild(ints);}var coms=ints.querySelector('.Qixian-pyq-coms');if(!coms){coms=document.createElement('div');coms.className='Qixian-pyq-coms';ints.appendChild(coms);}coms.insertAdjacentHTML('beforeend','<div class="Qixian-pyq-com"><span>'+finalRName+'</span>：'+t+'</div>');}Q('.Qixian-jpyqcommodal').classList.remove('show');Q('.Qixian-jpyqcomtxt').value='';}});
...
function checkCharSwitch(){
    try{
        ...
        if(charInfo.name && charInfo.name !== currentCharName){
            console.log('[Qixian Phone v3] 角色切换：'+currentCharName+' → '+charInfo.name);
            ...
            if(panel){
                ...
                var userInfo = getTavernUser();
                currentCharName = charInfo.name;
                runPhoneLogic(panel, charInfo, userInfo);
            }
        }
    }catch(e){}
}
