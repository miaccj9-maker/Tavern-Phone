...
function initPhone(scope, charInfo, userInfo){
    ...
    function resolveAvatar(rawUrl,isChar){
        ...
        try{
            if(isChar){
                var cImg=document.querySelector('#avatar,.char-avatar img,.avatar img,img.avatar,.character-avatar img,[class*="char-avatar"] img,[class*="character-avatar"] img,[class*="bot-avatar"] img,[class*="ai-avatar"] img,.mufy-char-avatar img,[data-testid*="avatar"] img,[class*="ChatMessage"] img');
                if(cImg&&cImg.src&&cImg.src.length>10) return cImg.src;
            }else{
                var uImg=document.querySelector('#user_avatar,.user-avatar img,.user-icon img,.user-avatar,.user-avatar-icon img,[class*="user-avatar"] img,.mufy-user-avatar img,[class*="self-avatar"] img,[class*="my-avatar"] img');
                if(uImg&&uImg.src&&uImg.src.length>10) return uImg.src;
            }
        }catch(e){}
        return rawUrl||'';
    }
    ...
    setTimeout(syncAvatarsFromPage,3000);
    setTimeout(syncAvatarsFromPage,5000);
...
function scanNodeForPhoneMsg2(node){
    ...
    for(var cni=0;cni<charNames.length;cni++){
        try{
            ...
            while((dm=dmPhoneMsgRe.exec(txt))!==null){if(dm[1].trim()){try{if(window.NcAddCallBubble)window.NcAddCallBubble('left',dm[1].trim(),false,'voice');}catch(e){}}}
    ...
    if(mePhoneMsg&&mePhoneMsg[1].trim()){try{if(window.NcAddCallBubble)window.NcAddCallBubble('right',mePhoneMsg[1].trim(),false,'voice');}catch(e){}}
