...
function getTavernChar(){
    try{
        ...
        var ch=document.querySelector('#avatar,.char-avatar img,.avatar img,img.avatar,[data-testid="char-avatar"],.character-avatar img');
        if(ch&&ch.src){ avatar=ch.src; if(!name) name=ch.alt||ch.getAttribute('title')||''; }
        if(!name){
            var nm=document.querySelector('.char-name,#ch_name,[data-testid="char-name"],.character-name');
            if(nm&&nm.textContent) name=nm.textContent.trim();
        }
        if(name) return {name:name,avatar:avatar};
    }catch(e){}
...
function getTavernUser(){
    try{
        ...
        var ua=document.querySelector('#user_avatar,.user-avatar img,.user-icon img,.user-avatar,[data-testid="user-avatar"],.user-avatar-icon img');
